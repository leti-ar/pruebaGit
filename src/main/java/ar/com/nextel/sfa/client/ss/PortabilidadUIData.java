		...
		if("POSTPAGO".equals(lstTipoTelefonia.getSelectedItemText())){
			...
			if(!fechaUltFactura.getTextBox().getText().equals("")) {
				if(!fechaUltFactura.validarFecha()) {
					validador.addError("El campo Portabilidad: Fecha de Emisi�n debe tener el siguiente formato dd/mm/yyyy");
				} else {
					Date fechaActual = new Date();
					if(fechaUltFactura.getFecha().compareTo(fechaActual) == 1) {
						validador.addError("El campo Portabilidad: Fecha de Emisi�n no debe ser mayor a la fecha actual");
					}
				}
			}
