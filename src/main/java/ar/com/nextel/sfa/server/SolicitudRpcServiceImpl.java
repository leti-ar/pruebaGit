...
//		avalonSystem = (AvalonSystem) context.getBean("avalonSystemBean");
	...
	public SolicitudServicioDto createSolicitudServicio(
		...
		//calculo los descuentos aplicados a cada l챠nea y se los seteo 
	...
	public SolicitudInitializer getSolicitudInitializer() {
		...
		//Se cargan todos los vendedores que no sean del tipo Telemarketer, Dae o Administrador de Cr챕ditos
	...
	public LineasSolicitudServicioInitializer getLineasSolicitudServicioInitializer(
		...
		List<TipoSolicitudDto> tiposSolicitudDeGrupoSelected = tiposSolicitudPorGrupo.get(grupoSolicitudDto
				.getId());
		// Si no es vaci占� (no deber占폸 serlo) carga la lista de precios del primer tipoSolicitud que se muestra
	...
	public List<ListaPreciosDto> getListasDePrecios(TipoSolicitudDto tipoSolicitudDto) {
		...
		// Se realiza el mapeo de la colecci占퐊 a mano para poder filtrar los items por warehouse
		for (ListaPrecios listaPrecios : listasPrecios) {
			ListaPreciosDto lista = mapper.map(listaPrecios, ListaPreciosDto.class);
			if (!activacion) {
				//MGR - #873 - Se agrega el Vendedor
	...
	public GeneracionCierreResultDto generarCerrarSolicitud(SolicitudServicioDto solicitudServicioDto,
		...
		String accion = cerrar ? "cierre" : "generaci처n";
	...
	public CreateSaveSSTransfResultDto createSolicitudServicioTranferencia(
		...
		if(vendLogeo.isADMCreditos() || vendLogeo.isAP()){
			...
			if(solicitud.getCuenta().getVendedorLockeo() != null &&
				...
				resultDto.addMessage("La cuenta est찼 lockeada por un ejecutivo, el supervisor es " + 
		...
		if(nroSS_portabilidad.contains("N")){ // Contiene la letra N
			if(nroSS_portabilidad.contains(".")){ // Contiene un punto
				...
				if(nroSS_portabilidad_aux.equals(nroSS)){ // Numero igual al de solicitud de servicio
					nroSS_portabilidad_aux = nroSS_portabilidad.substring(nroSS_portabilidad.lastIndexOf("."));
					try{
		...
		if(x_nroSS.size() > 0){
			for(int i = 0; i < x_nroSS.size(); i++){
				...
				for(int j = 0; j < x_nroSS.get(i).size(); j++){
					portabilidad = lineas.get(i).getPortabilidad();
					...
					for(int n = 0; n < x_nroSS.get(i).size(); n++){
						if(j != n){ // Solo compara si las lineas son de diferentes indice en la lista (no son la misma)
							if(solicitudServicioDto.getLineas().get(n).getPortabilidad() != null){
								portabilidadAUX = solicitudServicioDto.getLineas().get(n).getPortabilidad();
...
//			}
		...
		return new PortabilidadResult(true,false,"PRUEBAS RTF");
	}
	...
	public void generarRTFportabilidad(SolicitudServicioDto solicitudServicioDto) throws RpcExceptionMessages {
		PortabilidadPropertiesRTFGenerator portabilidadPropGen = 
			new PortabilidadPropertiesRTFGenerator(repository.retrieve(CuentaDto.class, solicitudServicioDto.getCuenta().getId()),solicitudServicioDto);
